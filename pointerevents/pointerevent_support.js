...
function getEvent(event_type, target, test) {
  return new Promise(resolve => {
    ...
    if (test) {
      test.add_cleanup(() =>
          target.removeEventListener(event_type, e => resolve(e), { once: true }));
    }
  });
}
...
function getMessageData(message_data_type, source, test) {
  return new Promise(resolve => {
    function removeAndResolve(e) {
      if (e.source != source || !e.data || e.data.type != message_data_type)
        return;
      window.removeEventListener("message", removeAndResolve);
      resolve(e.data);
    }

    window.addEventListener("message", removeAndResolve);
    if (test) {
      test.add_cleanup(() =>
          window.removeEventListener("message", removeAndResolve));
    }
...
function preventDefaultPointerdownOnce(target, test) {
  return new Promise((resolve) => {
    function cancelAndResolve(e) {
      e.preventDefault();
      resolve();
    }

    target.addEventListener("pointerdown", cancelAndResolve, { once: true });
    if (test) {
      test.add_cleanup(() =>
          target.removeEventListener("pointerdown", cancelAndResolve, { once: true }));
    }
  });
}
