...
const runTest = (description) => {
  ...
  promise_test((test) => {
    return new Promise(async resolve => {
      ...
      const child_unloaded = new Promise(r => window.popup.onunload = e => r());
      document.body.append(iframe);
      await child_unloaded;  // This makes sure the old child has unloaded, but
                             // with the timeout below it's really not needed.

      test.step_timeout(() => {
        resolve(); // This is necessary, or else the test times out. The
                   // about:blank load does not fire an onload event we can
                   // access.
      }, 500);
    }).then(() => {
